...
def token_logout(request):
    ...
    try:
        ...
        try:
            ...
            cursor.execute(f"UPDATE app_useraccount SET last_logout = '{current_time}' WHERE id = '{user_id}'")
...
@api_view(['POST'])
@authentication_classes([JWTAuthentication])  # Sử dụng JWTAuthentication để xác thực
...
def message_list(request):
    if request.method == 'GET':
        messages = Message.objects.all().order_by('created_at')
        data = [{'id': str(message.id), 'text': message.text, 'created_at': message.created_at.isoformat()} for message in messages]
        return JsonResponse(data, safe=False)
        ...
        return JsonResponse({'id': str(message.id), 'text': message.text, 'created_at': message.created_at.isoformat()})
